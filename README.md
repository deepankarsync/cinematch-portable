...
> [!IMPORTANT]
> **Embeddings Generation**: The `description_embeddings.pkl` file must be generated by running the embedding computation cell in the notebook. This file is required for both the offline evaluation and the real-time CF Frontend. Without it, the "first-stage retrieval" will fail.
...
├── cf-frontend/                      # FastAPI + React web application
│   ├── app.py                        # FastAPI backend
│   ├── cinematch-frontend/           # React frontend (static export)
...
## User Interface (CF Frontend)

The project includes a web-based UI for real-time recommendations.

### 1. Prerequisites

The frontend requires pre-computed artifacts from the notebook. You **must** run the `ultimate_cinematch.ipynb` notebook at least once to generate the following files in the `cache/` directory:
- `movieid_metadata_ultimate.pkl`
- `movieid_list.pkl`
- `description_embeddings.pkl` (This is critical for the first-stage retrieval)
...
# Copy the generated cache and links.csv to the frontend directory
cp -r ../cache .
cp ../links.csv .
# Start the backend
python app.py
